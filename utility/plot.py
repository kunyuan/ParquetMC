...
def Errorbar(x, d, err=None, fmt='o-', capthick=1, capsize=4, color=None, label=None, size=4, shift=False):
    ax = plt.gca()
    if err is None:
        err = d*0.0
    return ax.errorbar(x, d, yerr=err, fmt='o-',
                       capthick=1, capsize=4, color=color, label=label)
