...
vector<double> grid::uniLogGrid(const vector<UniLog> &Unilog,
                                const vector<array<int, 2>> &Range) {
...
void UniLog::init(array<double, 2> _bound, int _init, int _m, int _n,
                  double _alpha) {
  bound = _bound;
  assert(bound[0] < bound[1]);
...
double UniLog::grid(int i) const {
  if (d2s) {
    ...
    int i_m = (i - idx[0]) / n;
    if (i_m == 0)
      return bound[0] + pow(alpha, m) / n * length * i_n;
    return bound[0] + pow(alpha, m + 1 - i_m) * length +
           (pow(alpha, m - i_m) - pow(alpha, m - i_m + 1)) / n * i_n * length;
  } else {
    int i_n = (idx[0] - i) % n;
    int i_m = (idx[0] - i) / n;
    if (i_m == 0)
...
int UniLog::floor(double x) const {
  ...
  int i_n = (norm - pow(alpha, i_m + 1)) /
            (pow(alpha, i_m) - pow(alpha, i_m + 1)) * n;
  if (d2s)
